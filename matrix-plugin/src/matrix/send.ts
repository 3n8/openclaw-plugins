...
export async function reactMatrixMessage(
  ...
  messageId: string,
  emoji: string,
  clientOrAccountId?: MatrixClient | string,
  accountId?: string,
): Promise<void> {
  if (!emoji.trim()) {
  ...
  const { client: resolved, stopOnDone } = await resolveMatrixClient({
    client: typeof clientOrAccountId === "object" ? clientOrAccountId : undefined,
    accountId: typeof clientOrAccountId === "string" ? clientOrAccountId : accountId,
  });
  try {
    const resolvedRoom = await resolveMatrixRoomId(resolved, roomId);
